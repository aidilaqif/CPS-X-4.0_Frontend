...
const LocationOverview = ({ locations, items }) => {
  ...
  const getLocationStats = (locationId) => {
    ...
    const itemCounts = {
      ...
      pallets: locationItems.filter((item) => item.label_type === "FG Pallet")
        .length,
    };
    ...
    <div className="location-overview">
        ...
        <div className="filter-buttons">
          ...
          <button
            className={`filter-btn ${
              selectedType === "Paper Roll Location" ? "active" : ""
            }`}
          ...
          <button
            className={`filter-btn ${
              selectedType === "FG Location" ? "active" : ""
            }`}
      ...
      <div className="location-overview-grid">
        {filteredLocations.map((location) => {
          ...
          const isOldScan =
            stats.lastScan && moment().diff(stats.lastScan, "days") > 7;

          return (
            <div
              key={location.location_id}
              className={`location-card ${
                isOldScan ? "location-card-outdated" : "location-card-recent"
              }`}
            >
